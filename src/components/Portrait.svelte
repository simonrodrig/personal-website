<script lang=ts>
    ...
    const randChoice = <T,>(lst: T[]): T => lst[Math.floor(Math.random() * lst.length)]
    ...
    let currBackShape = randChoice(shapes);

    setInterval(() => {
        currShape = newShape(currShape, shapes);
        currBackShape = newShape(currBackShape, shapes);
    }, 500)
...
<div class="container">
    <svg class="background-blob" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
      <path fill={outerLayerColor} d={currBackShape} transform="translate(100 100) scale(1.2)"/>
    </svg>

    <svg class="foreground-blob" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
        <path fill={innerLayerColor} d={currShape} transform="translate(100 100) scale(1.2)" />
    </svg>

    <span class="slot-contain">
        <slot></slot>
    </span>
</div>
